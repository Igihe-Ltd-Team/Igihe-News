...
export async function prefetchHomeData(): Promise<HomePageData> {
  try {
    ...
    const [
      ...
      ApiService.fetchArticles({ tags: [80], per_page: 8 }),
      ApiService.fetchMostPopularArticlesFallback({ period: 'week', limit: 5 }),
      ApiService.fetchArticles({ tags: [133], per_page: 11, orderby: 'date' }),
...
export async function prefetchBelowFoldData() {
  try {
    const [
      ...
      ApiService.fetchArticles({ tags: [99], per_page: 12 }),
    ...
    return {
      ...
      africaArticles: greatLakesResponse.status === 'fulfilled' ? greatLakesResponse.value.data : [],
