...
export const getTags = (articleData: NewsItem) => {
  if (articleData?._embedded && articleData?._embedded['wp:term'] && articleData?._embedded['wp:term'][1]) {
    return articleData?._embedded['wp:term'][1];
  }
  return [];
};
